...
export default function RepoDetails({ repo, archive }: { repo: tBorgRepo, archive?: string }) {
  ...
  const [expanded, setExpanded] = React.useState<string | false>(default_backup);
  return (
    <>
      ...
      <CardContent>
          ...
          <Table>
            <TableBody>
              <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell
                  align="right" sx={{ color: get_status_color(repo.last_run?.status) }}
        ...
        {repo.logspath &&
          <Button
            ...
            onClick={() => { router.push(`/logs/${repo.name}`); }}
          >
            Go to logs
          </Button>
        }
  ...
  return (
    <TableContainer>
      <Table>
        <TableBody>
          <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
            <TableCell component="th" scope="row" align="left">Original size</TableCell>
            <TableCell component="th" scope="row" align="center">Compressed size</TableCell>
